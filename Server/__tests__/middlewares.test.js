...
describe("errorHandler", () => {
  ...
  it("handles fk and generic", () => {
    ...
    const r3 = res();
    errorHandler({ name: "MulterError", code: "LIMIT_FILE_SIZE" }, {}, r3, jest.fn());
    expect(r3.status).toHaveBeenCalledWith(400);
    expect(r3.json).toHaveBeenCalledWith({ error: "Ukuran foto maksimal 5 MB" });
    const r4 = res();
    errorHandler({ name: "MulterError", code: "LIMIT_UNEXPECTED_FILE" }, {}, r4, jest.fn());
    expect(r4.json).toHaveBeenCalledWith({ error: "Gagal mengunggah foto" });
    const r5 = res();
    errorHandler({ type: "entity.parse.failed" }, {}, r5, jest.fn());
    expect(r5.status).toHaveBeenCalledWith(400);
    const parseErr = new SyntaxError("Unexpected token");
    parseErr.status = 400;
    const r6 = res();
    errorHandler(parseErr, {}, r6, jest.fn());
    expect(r6.json).toHaveBeenCalledWith({ error: "Data tidak valid" });
  });
});
